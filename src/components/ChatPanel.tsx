...
export default function ChatPanel({ state }: { state: { hunger: number; mood: number; energy: number } }) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ message: userMsg, state })
      });
      const data = await res.json();

      if (data.reply) {
        setMessages(prev => [...prev, { id: Date.now().toString(), sender: 'gochi', text: data.reply }]);
      }
    } catch (error) {
      console.error(error);
    } finally {
